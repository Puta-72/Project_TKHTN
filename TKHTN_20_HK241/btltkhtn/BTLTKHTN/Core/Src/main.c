...
uint8_t DHT11_Read (void)
{
  ...
  for (a=0;a<8;a++)
  {
    ...
    cMillis = HAL_GetTick();
    while (!(HAL_GPIO_ReadPin (DHT11_PORT, DHT11_PIN)) && pMillis + 2 > cMillis)
    {  // Doi cho pin len muc cao
      cMillis = HAL_GetTick();
    }
    microDelay (40);   // doi 40 us
